...
export function ResultForm({ action, indicators, result, defaultIndicatorId }: { action: (formData: FormData) => void; indicators: any[]; result?: any; defaultIndicatorId?: string }) {
  ...
  return <form action={action} className="card form"><label>Indicador<select className="select" name="indicatorId" defaultValue={result?.indicatorId??defaultIndicatorId}>{indicators.map(i=><option key={i.id} value={i.id}>{i.code} - {i.name}</option>)}</select></label><div className="grid grid-4"><label>Ano<input className="input" name="year" type="number" defaultValue={reference.getUTCFullYear()} required/></label><label>Mês<input className="input" name="month" type="number" min="1" max="12" defaultValue={reference.getUTCMonth()+1} required/></label><label>Resultado<input className="input" name="actualValue" type="number" step="0.01" defaultValue={result?.actualValue??""} required/></label><label>Meta<input className="input" name="targetValue" type="number" step="0.01" defaultValue={result?.targetValue??""} required/></label></div><label>Análise<textarea className="textarea" name="analysis" defaultValue={result?.analysis??""}/></label><label>Plano de ação<textarea className="textarea" name="actionPlan" defaultValue={result?.actionPlan??""}/></label><button className="button">Salvar</button></form>;
}
