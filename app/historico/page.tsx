...
export default async function HistoryPage() {
  ...
  return (
    <>
      ...
      <section className="card"><div className="table-wrap"><table className="table"><thead><tr><th>Data</th><th>Entidade</th><th>Ação</th><th>Resumo</th><th>Usuário</th><th>Perfil</th></tr></thead><tbody>{logs.map(log=><tr key={log.id}><td>{log.createdAt.toLocaleString("pt-BR")}</td><td>{log.entity}</td><td>{log.action}</td><td>{log.summary}</td><td>{log.actorName}</td><td>{formatRole(log.actorRole)}</td></tr>)}</tbody></table></div></section>
    </>
  );
}
