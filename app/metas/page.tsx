...
export default async function GoalsPage() {
  ...
  return (
    <>
      <section className="page-header"><div><h2>Metas</h2><p>CRUD completo de metas mensais, trimestrais e anuais.</p></div><div className="inline-actions"><Link className="button secondary" href="/metas/export/excel"><FileSpreadsheet aria-hidden="true" size={17} />Excel</Link><Link className="button secondary" href="/metas/export/pdf"><FileText aria-hidden="true" size={17} />PDF</Link><Link className="button" href="/metas/novo">Nova meta</Link></div></section>
      <section className="card"><div className="table-wrap"><table className="table"><thead><tr><th>Indicador</th><th>Ano</th><th>Trimestre</th><th>Mês</th><th>Meta</th><th>Alerta</th><th>Mínimo</th><th>Ideal</th><th>Ações</th></tr></thead><tbody>{goals.map(goal=><tr key={goal.id}><td>{goal.indicator.code} - {goal.indicator.name}</td><td>{goal.year}</td><td>{goal.quarter??"-"}</td><td>{goal.month??"-"}</td><td>{goal.targetValue}</td><td>{goal.alertValue??"-"}</td><td>{goal.minimumValue??"-"}</td><td>{goal.idealValue??"-"}</td><td><div className="table-actions"><Link className="button small secondary" href={`/metas/${goal.id}/editar`}>Editar</Link><form action={deleteGoal.bind(null,goal.id)}><SubmitButton className="button small danger" pendingLabel="Excluindo...">Excluir</SubmitButton></form></div></td></tr>)}</tbody></table></div></section>
    </>
  );
}
